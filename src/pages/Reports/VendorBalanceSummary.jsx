...
const VendorBalanceSummary = () => {
  document.title = "Vendor Balance Summary";
  ...
  const fetchReport = async (asOfDate) => {
    ...
    setLoading(true);
    try {
      let url = `v1/buildings/${buildingId}/reports/vendor-balance-summary?as_of_date=${asOfDate}`;
      const { data } = await axiosInstance.get(url);
      setReport(data.data);
    } catch (error) {
      toast.error(error.response?.data?.error || "Failed to fetch report");
      console.error("Error fetching vendor balance summary:", error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handlePrint = () => {
    ...
    document.title = `Vendor Balance Summary As of ${dateStr}`;
    window.print();
  ...
  return (
    <React.Fragment>
      ...
      <div className="page-content">
        <Container fluid>
          <Breadcrumbs title="Vendor Balance Summary" breadcrumbItem="Vendor Balance Summary" />
          <Row>
            <Col xs={12}>
              <Card>
                <CardBody>
                        ...
                        <Table bordered striped style={{ width: "60%" }}>
                          ...
                          <tbody>
                            {report.vendors.map((vendor, index) => (
                              <tr key={`vendor-${vendor.people_id}-${index}`}>
                                <td>{vendor.people_name}</td>
                ...
                <Card>
                  <CardBody>
                    <div className="text-center">
                      <p>No vendor balances found for the selected date.</p>
                    </div>
                  </CardBody>
                </Card>
              </Col>
