...
const EditLease = () => {
  document.title = "Edit Lease";
  const { id: buildingId, leaseId } = useParams();
      ...
      try {
        ...
        if (newFiles.length > 0) {
          for (const file of newFiles) {
            await uploadFile(parseInt(leaseId), file);
          }
        }

        navigate(`/building/${buildingId}/leases`);
      } catch (error) {
        const errorMsg = error.response?.data?.error || "Failed to update lease";
  ...
  const fetchLeaseForEdit = async () => {
    try {
      ...
      validation.setValues({
        ...
        start_date: lease.start_date ? moment(lease.start_date).format("YYYY-MM-DD") : moment().format("YYYY-MM-DD"),
        end_date: lease.end_date ? moment(lease.end_date).format("YYYY-MM-DD") : "",
  ...
  useEffect(() => {
    ...
    if (leaseId) {
      ...
      // For create mode, fetch all available units
      fetchUnits();
    }
  }, [buildingId, leaseId]);
  ...
  return (
    <React.Fragment>
      <div className="page-content">
        <Container fluid>
          <Breadcrumbs title="Edit Lease" breadcrumbItem="Edit Lease" />
          <Row>
            <Col lg="12">
              <Card>
                <CardBody>
                  ...
                  >
                    <Row>
                      ...
                      <Col md={12}>
                        <div className="mb-3">
                          <Label>Add New Files (Optional)</Label>
                          <Input
                            type="file"
                            multiple
